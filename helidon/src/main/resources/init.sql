...
CREATE TABLE IF NOT EXISTS fruits (id BIGINT PRIMARY KEY DEFAULT nextval('fruits_seq'), name VARCHAR(255) NOT NULL UNIQUE, description VARCHAR(255));
CREATE TABLE IF NOT EXISTS stores (id BIGINT PRIMARY KEY DEFAULT nextval('stores_seq'), name VARCHAR(255) NOT NULL UNIQUE, address VARCHAR(255) NOT NULL, city VARCHAR(255) NOT NULL, country VARCHAR(255) NOT NULL, currency VARCHAR(255) NOT NULL);
CREATE TABLE IF NOT EXISTS store_fruit_prices (store_id BIGINT NOT NULL, fruit_id BIGINT NOT NULL, price NUMERIC(12, 2) NOT NULL, PRIMARY KEY (store_id, fruit_id), CONSTRAINT fk_store_fruit_prices_store FOREIGN KEY (store_id) REFERENCES stores (id), CONSTRAINT fk_store_fruit_prices_fruit FOREIGN KEY (fruit_id) REFERENCES fruits (id));
